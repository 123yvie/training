...
@allure.step("步骤1 ==>> 创建案例")
def step_1():
    logger.info("步骤1 ==>> 创建案例")


@allure.epic("Tips_Case")
@allure.feature("Tips_Case - 创建案例")
@pytest.mark.usefixtures('init_fixture')  # 获取token
class TestPaymentOverview:
    @allure.story('test_tips_create_case接口')
    @pytest.mark.parametrize("case_data", render_yaml(os.path.join(BASE_PATH, 'tips', 'test_tips_create_case.yaml')))  # 把yaml文件的参数赋值给case_data
